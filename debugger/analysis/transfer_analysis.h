...
enum class AnalysisStage { Effects, Continuations, Values, Constructed, Stack };
// Occurrences are retained alongside grouped edges; destination sets stay open.
[[nodiscard]] Result<std::string> TransferReport(const TransferCapture& capture, AnalysisStage through = AnalysisStage::Stack);
